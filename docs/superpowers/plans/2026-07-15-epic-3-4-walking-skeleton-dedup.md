...
**Kết quả thực hiện (2026-07-21):**

Verify bằng PR test thật (#30, `nhd98z2/Happyfeeling`), fixture 4 lỗi cố ý (`scripts/aic34-verify-fixture.js`, đã xoá trước khi đóng PR test #30) — dùng chung cách tự dựng request HMAC hợp lệ như AIC-31 (GitHub App vẫn thuộc account Bằng, không đổi được webhook URL sang môi trường local).

- **Test chính (redeliver thật — cùng `headSha`, gửi 2 lần):** gửi 2 request y hệt tới `/api/webhook`. Log worker chỉ ghi nhận **1 lần** pipeline chạy (BullMQ, AIC-33, chặn job thứ 2 ngay từ bước enqueue vì `jobId` giống hệt — job không bao giờ chạm tới bước dedup nội dung). Kết quả: đúng 4 comment khớp 4 finding thật, đúng 4 row `Finding`, không trùng lặp — **PASS**.
- **Test phụ (không thuộc tiêu chí gốc, tự thêm để verify riêng AIC-32):** enqueue trực tiếp 2 job có `jobId` khác nhau nhưng cùng nội dung (owner/repo/prNumber/headSha), để buộc pipeline chạy đủ 2 lần, bỏ qua lớp chặn của AIC-33. Bằng chứng: 3/4 finding cũ được `filterNewFindings` nhận diện đúng, bump `lastSeenAt` thay vì post lại comment mới — xác nhận AIC-32 hoạt động đúng độc lập với AIC-33. Finding thứ 4 không phải bug ở AIC-32: `computeDedupHash()` (`packages/github/src/review/dedup.ts`) tính hash từ `repo:prNumber:filePath:line` — không có nội dung message. Do bug line-anchor đã ghi ở mục dưới (Groq không set `temperature`, cho ra `line` khác nhau giữa các lần gọi cho cùng 1 bug), rất có thể lần chạy pipeline thứ 2 Groq trả về `line` khác lần đầu cho đúng 1 finding → hash đổi → `filterNewFindings` coi đây là finding mới theo đúng thiết kế hash-theo-line của nó. Đây là hệ quả của bug line-anchor đã biết, không phải lỗi trong logic dedup.
- **Sự cố thao tác gặp phải (đã xử lý, không phải bug code):** lần chạy test đầu tiên dùng nhầm Docker image `web`/`worker` cũ (build tối hôm trước AIC-32 merge) do `docker compose up -d` không tự rebuild — phải `docker compose build web worker` lại rồi obliterate queue (BullMQ giữ lịch sử job theo `jobId` cũ) trước khi test lại cho đúng code mới nhất.
- **Phát hiện phụ, ngoài phạm vi Epic 4:** bot gắn sai dòng cho comment trên PR test (nội dung finding đúng, chỉ sai vị trí neo trên diff — thường lệch 1-2 dòng, rơi vào dòng đóng ngoặc `}` của khối chứa bug thay vì đúng statement gây lỗi). Nguyên nhân: `buildPrompt()` (`llmReviewer.groq.ts`) gửi raw diff thô cho Groq, không đánh số dòng kèm theo và không dặn cách tính `line` trả về — model phải tự đếm từ text, vốn là việc LLM làm không chính xác. Đã loại trừ nguyên nhân do `chunkDiff` (diff nhỏ, chỉ 1 chunk, đầy đủ context, không bị cắt vỡ). Request gọi Groq cũng không set `temperature` → không deterministic: verify lại 1 lần nữa trên PR demo riêng (#31, cùng fixture y hệt PR #30) cho kết quả **khác hẳn** — PR #30 đúng 1/4, PR #31 sai cả 4/4 (kể cả bug giống hệt nhau giữa 2 PR cũng bị neo lệch dòng khác nhau mỗi lần) — bằng chứng rõ đây là lỗi hệ thống (thiếu chỉ dẫn cho model), không phải may rủi riêng lẻ. Không liên quan tới dedup/idempotency. Đã note trên PR #30 (comment tiếng Anh) + để PR #31 mở kèm comment thật cho Bằng xem trực tiếp trước khi đóng, báo Bằng qua Telegram — không xử lý trong task này.
- `AIC-8` (Epic 4) đã chuyển Done trên Linear.
