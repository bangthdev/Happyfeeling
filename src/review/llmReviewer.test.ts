...
describe('reviewDiff', () => {
  it('parses findings from a valid tool_use response', async () => {
      ...
      content: [
        {
          ...
          input: {
            findings: [{ file: 'src/x.ts', line: 10, severity: 'high', message: 'bug', suggestion: 'fix it' }],
          },
        },
      ],
    ...
    const result = await reviewDiff({ diff: 'diff...', files: ['src/x.ts'] }, client);
  ...
  it('retries once when the first response has no tool_use block', async () => {
    ...
    const client = fakeClient(async () => {
      callCount += 1;
      if (callCount === 1) {
        return { content: [{ type: 'text', text: 'oops' }], usage: { input_tokens: 10, output_tokens: 5 } };
      }
      return {
        content: [{ type: 'tool_use', name: 'submit_findings', input: { findings: [] } }],
