...
if (!API_KEY) {
  throw new Error('Missing LINEAR_API_KEY in .env — tạo Personal API Key ở Linear Settings > API rồi thêm vào .env');
}
...
async function findTeamId(): Promise<string> {
  const data = await linearRequest<{ teams: { nodes: { id: string; name: string; key: string }[] } }>(
...
async function findOrCreateLabelIds(teamId: string): Promise<string[]> {
  const data = await linearRequest<{ issueLabels: { nodes: { id: string; name: string }[] } }>(
  ...
  for (const name of LABEL_NAMES) {
    ...
    const created = await linearRequest<{ issueLabelCreate: { success: boolean; issueLabel: { id: string } } }>(
...
async function findIssueIdByTitle(teamId: string, title: string): Promise<string | null> {
  const data = await linearRequest<{ issues: { nodes: { id: string; identifier: string }[] } }>(
...
async function getIssueById(id: string): Promise<{ id: string; identifier: string; url: string }> {
  const data = await linearRequest<{ issue: { id: string; identifier: string; url: string } }>(
...
async function updateIssue(
  ...
  const data = await linearRequest<{
    ...
    { id, title: epic.title, description: epic.description, priority: epic.priority, labelIds },
  );
  console.log(`Đã đổi số: ${data.issueUpdate.issue.identifier} — "${epic.oldTitle}" → "${epic.title}"`);
...
async function createIssue(
  ...
  const data = await linearRequest<{
    ...
    { teamId, title: epic.title, description: epic.description, priority: epic.priority, labelIds },
  );
...
async function blockRelationExists(blockerId: string, blockedId: string): Promise<boolean> {
  const data = await linearRequest<{
    issue: { relations: { nodes: { type: string; relatedIssue: { id: string } }[] } };
  }>(
  ...
  return data.issue.relations.nodes.some((r) => r.type === 'blocks' && r.relatedIssue.id === blockedId);
}

async function createBlockRelation(blockerId: string, blockedId: string): Promise<void> {
...
async function main() {
  ...
  const created = new Map<number, { id: string; identifier: string; url: string }>();
  ...
  console.log('Đã set xong quan hệ "blocks" giữa các epic theo bảng phụ thuộc.');
